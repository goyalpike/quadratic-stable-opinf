...
path_funcs = {
    "no_hypos": ("./Results/Burgers_Neumann/NoStability/", module_models.ModelHypothesis, True),
    "localstability": (
        "./Results/Burgers_Neumann/LocalStability/",
        module_models.ModelHypothesisLocalStable,
        False,
    ),
    "globalstability": (
        "./Results/Burgers_Neumann/GlobalStability/",
...
data = loadmat("./Burgers_data/burgers_data_Neumann.mat")
# A = data["Af"]
# H = data["H"]
...
# train_idxs = list([1,2,3,4,5])
# testing_idxs = train_idxs
...
reduced_orders = np.arange(8, 21, 2)
reduced_orders = np.arange(16, 17, 2)
...
for r in reduced_orders:
	...
	PATH = Params.path
	for reg_h in Reg_H:
		Params.path = PATH + str(int(np.log10(reg_h))) +'/'
		if not os.path.exists(Params.path):
			os.makedirs(Params.path)
		print(Params.path)
		print(reg_h)
		...
		for k in range(X_testing.shape[0]):
			...
			if (sol_OpInf.y).shape[-1] == len(t_testing):
				...
				# Plot the surface.
				surf = ax[0].plot_surface(x, y_testing, X_testing[k].T, cmap=cm.RdYlGn)
				surf = ax[1].plot_surface(
					x, y_testing, scaling_fac * full_sol_OpInf.T, cmap=cm.RdYlGn
				)
